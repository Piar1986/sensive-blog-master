...
def index(request):
    ...
    fresh_posts = Post.objects.order_by('-published_at').prefetch_related('author', Prefetch('tags', queryset=Tag.objects.annotate(Count('posts'))))
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all()[:20].prefetch_related('author', Prefetch('tags', queryset=Tag.objects.annotate(Count('posts')))).fetch_with_comments_count()
